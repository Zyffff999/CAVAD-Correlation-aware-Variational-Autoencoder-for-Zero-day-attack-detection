...
class SessionEncoder(nn.Module):
    ...
    def __init__(self, packet_latent_dim=256, d_model=256, latent_dim=32, spatial_reduction_size=8,
                 header_only=False,
                 use_dilation=True,
                 depthwise_only=False,):
        ...
        self.final_norm = nn.LayerNorm(session_tcn_channels[-1])
    ...
    def forward(self, sessions):
        # sessions: [B, num_packets(=16), packet_len(=256)]
        B, N, L = sessions.shape
        packets_flat = sessions.reshape(-1, L)  # [B*N, 256]
        z_flat = self.packet_encoder(packets_flat)  # [B*N, d_model]

        z_seq = z_flat.view(B, N, -1).permute(0, 2, 1)  # [B, d_model, 16]
        session_feat = self.session_tcn(z_seq)  # [B, 64, 16]
...
class FactorizedDecoder(nn.Module):
    def __init__(self, latent_dim=96, output_size=16 * 256, rank=256, num_deep_layers=2):
        super().__init__()
