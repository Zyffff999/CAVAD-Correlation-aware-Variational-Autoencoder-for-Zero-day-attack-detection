...
from model.packet import PacketEncoder, FactorizedDecoder, SessionEncoder
...
class CorrelatedGaussianVAE(nn.Module):
    ...
    def get_features(self, x: torch.Tensor):
        with torch.no_grad():
            x = x.float()
            packet_features = self.encoder(x)
            bottleneck_features = self.bottleneck_fc(packet_features)
            mu = self.fc_mu(bottleneck_features)
            return packet_features, mu

    def compute_mahalanobis_distance(self, x: torch.Tensor, reference_mu: torch.Tensor, reference_cov_inv: torch.Tensor):
        _, mu = self.get_features(x)
